...
BUILD_ID = "K230_ROAD_THREE_BAND_GEOMETRY_FIX_20260725_02"
...
# === three-band compatibility aliases ===
# Keep old/new band naming compatible after conversion.
UPPER_BAND_TOP = 70
UPPER_BAND_BOTTOM = 105
MIDDLE_BAND_TOP = 105
MIDDLE_BAND_BOTTOM = 145
LOWER_BAND_TOP = 145
LOWER_BAND_BOTTOM = 220
LOWER_CONTROL_ROWS = set()  # filled after row generation
# === three-band row count configuration ===
# Sampling is fixed to upper 4 + middle 3 + lower 5 = 12 rows.
UPPER_SAMPLE_ROWS = 4
MIDDLE_SAMPLE_ROWS = 3
LOWER_SAMPLE_ROWS = 5
NUM_SAMPLE_ROWS = UPPER_SAMPLE_ROWS + MIDDLE_SAMPLE_ROWS + LOWER_SAMPLE_ROWS
...
# 道路几何 — 采样行
# Three-band sampling: UPPER 4 + MIDDLE 3 + LOWER 5 = 12 rows
# Recovery/state parameters
ENABLE_SINGLE_SIDE_RECOVERY = True
...
class RoadBoundaryExtractor:
    def __init__(self, detect_w=320, detect_h=240,
        ...
        self.upper_sample_y = self._make_band_rows(
            UPPER_BAND_TOP, UPPER_BAND_BOTTOM, UPPER_SAMPLE_ROWS)
        self.middle_sample_y = self._make_band_rows(
            MIDDLE_BAND_TOP, MIDDLE_BAND_BOTTOM, MIDDLE_SAMPLE_ROWS)
        self.lower_sample_y = self._make_band_rows(
            LOWER_BAND_TOP, LOWER_BAND_BOTTOM, LOWER_SAMPLE_ROWS)

        global LOWER_CONTROL_ROWS
        LOWER_CONTROL_ROWS = set(self.lower_sample_y)

        # Keep legacy names for protocol/display compatibility.
        self.lookahead_sample_y = self.upper_sample_y + self.middle_sample_y
        self.control_sample_y = self.lower_sample_y
        self.sample_y = (
            self.upper_sample_y +
            self.middle_sample_y +
            self.lower_sample_y
        )
...
class RoadGeometry:
        ...
        if boundary.left_valid and boundary.right_valid:
            ...
            for (lx, ly) in lp:
                ...
                if rx is not None:
                    ...
                    # Control input is the lower 5 rows only.
                    # Do not use the legacy CONTROL_BAND range here.
                    if ly in LOWER_CONTROL_ROWS:
    ...
    def _compute_heading(self, centers):
        ...
        if abs(dy) < 1.0:
            return 0.0
        # Sign convention:
        # road center moves to image right ahead -> heading_error > 0
        # keep heading and lateral error directions consistent for controller.
        dx = far_avg_x - near_avg_x
        ...
        try:
            ...
            while True:
                ...
                if self.mode_fsm.mode in (MODE_TURNING, MODE_NUMBER):
                    # TURNING still uses current image geometry. Never drive
                    # with the pre-turn last_geometry.
